...
async def fetch_rendered_html_with_scroll_async(list_url):
    async with async_playwright() as p:
        ...
        try:
            ...
            await context.close()
def fetch_rendered_html_with_scroll(SINGER_LIST_URL):
    return asyncio.run(fetch_rendered_html_with_scroll_async(SINGER_LIST_URL))

#playwright 爬取评论
async def fetch_comments_async(song_url, limit = COMMENT_LIMIT):
    async with async_playwright() as p:
        comments = []
        ...
        try:
            ...
            await context.close()
def fetch_comments(song_url):
    return asyncio.run(fetch_comments_async(song_url))
#改进版本，只需要在主逻辑中打开一次浏览器
...
def parse_singer_list_html(html_text, limit = SINGER_LIMIT):
    ...
    return singers
#第一次改进：
def crawl_singer_list(limit = SINGER_LIMIT):
    html_text = fetch_rendered_html_with_scroll(SINGER_LIST_URL)
    time_sleep()
...
#第三层：歌曲主页
#-------------------------------------------------------------------------
def crawl_song_info(song_dic, with_comments = True):
    ...
    if with_comments:
        comments = fetch_comments(SONG_URL)
            ...
            try:
                ...
                for singer_dic in singer_list:
                    ...
                    for song_dic in songs:
                        ...
                        song_info = crawl_song_info(song_dic, with_comments=False)
